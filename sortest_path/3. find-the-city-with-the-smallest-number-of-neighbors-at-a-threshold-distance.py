...
#%%
# core-concept
# j gular edge threshold thake beshi, oigula by-pass to dure thak direct e aser score nai
# this optimization greatly reduses adj list/matrix
class Solution:
    def findTheCity(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:  
        ...
        heap=[]
        result=[0]*n
        for i,j,k in edges: #i is 1st index, j is 2nd index,k is distance
            if k<=distanceThreshold:
                heappush(heap,(i,k,j))
                heappush(heap,(j,k,i))
                openset[i].append([j,k])
                openset[j].append([i,k])
        
        while (heap):
            # print("result:",result," heap:",heap)
            i,k,j=heappop(heap) #i is 1st index, j is 2nd index,k is distance
            if (i,j) not in closeset:
                closeset[(i,j)]=0
                if k<=distanceThreshold:
                    result[i]+=1
                    for u,v in openset[j]:#u is index, v is distance
                        if u!=i and k+v<=distanceThreshold:
                            heappush(heap,(i,k+v,u))
                            heappush(heap,(u,k+v,i))

        # return n-1-result[::-1].index(min(result))
        idx=0
        temp=result[0]
        for i in range(1,n):
            if result[i]<=temp:
